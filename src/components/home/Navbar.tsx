      ...
      {open && (
        <>
          ...
          <motion.div
            ...
            className="absolute top-full left-3 right-3 sm:left-4 sm:right-4 mt-2 md:hidden rounded-2xl overflow-hidden border border-emerald-500/20 shadow-2xl z-50 pointer-events-auto"
            style={{
          ...
          >
            <nav aria-label="Mobile navigation" className="p-3 sm:p-4">
              {hasUser && (
                <div className="flex items-center justify-between px-3 py-1.5 mb-2 rounded-lg bg-emerald-500/10 border border-emerald-500/20">
                ...
                {NAV_LINKS.map(({ label, href }) => (
                  <li key={label}>
                    <Link
                      href={href}
                      onClick={onClose}
                      className="flex items-center justify-between px-4 py-2.5 text-sm font-medium text-white/80 rounded-xl hover:text-white hover:bg-white/10 font-[family-name:var(--font-inter)] transition-colors duration-150"
                    >
                      <span>{label}</span>
